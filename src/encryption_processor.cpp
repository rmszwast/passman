#include "encryption_processor.hpp"

Encryptor::Encryptor(){};

// Derive key for encryption algorithm using salt
Botan::secure_vector<uint8_t>
Encryptor::deriveKey(std::string_view password, std::span<const uint8_t> salt,
                     size_t keyLength) {
...
Encryptor::setUpAEAD(std::string_view password, std::span<const uint8_t> salt,
...
std::vector<uint8_t> Encryptor::encrypt(std::string_view password,
                                        std::string_view stringToEncrypt) {
  ...
  // Convert string into data type Botan can work with
...
Botan::AutoSeeded_RNG Encryptor::generateRandomSeed() {
